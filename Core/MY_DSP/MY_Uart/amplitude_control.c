 ...
 * code=1024 -> 4460mV
 * code=512  -> 2240mV
 * 线性关系：mv = (2220/512)*code + 20
 * 反算关系：code = (mv - 20) * 512 / 2220
 */
#define AMPLITUDE_CAL_MV_OFFSET     20U
#define AMPLITUDE_CAL_CODE_NUM      512U
#define AMPLITUDE_CAL_MV_DEN        2220U
#define AD9959_AMPLITUDE_MAX_CODE   1023U
...
static uint32_t AmplitudeVoltageToParam(uint32_t amplitude_mv)
{
    ...
    // 两点线性标定反算并四舍五入：code = (mv - 20) * 512 / 2220
    if (amplitude_mv <= AMPLITUDE_CAL_MV_OFFSET)
    {
        return 0U;
    }

    uint64_t scaled = ((uint64_t)(amplitude_mv - AMPLITUDE_CAL_MV_OFFSET) *
                       (uint64_t)AMPLITUDE_CAL_CODE_NUM) +
                      ((uint64_t)AMPLITUDE_CAL_MV_DEN / 2U);
    uint32_t write_amplitude_param =
        (uint32_t)(scaled / (uint64_t)AMPLITUDE_CAL_MV_DEN);

    // AD9959 驱动接口幅值码上限 1023，超限时直接钳位
    if (write_amplitude_param > AD9959_AMPLITUDE_MAX_CODE)
    {
        write_amplitude_param = AD9959_AMPLITUDE_MAX_CODE;
    }
