...
stopFreq   = 3000;    % 终止频率 (Hz)
stepFreq   = 100;     % 步进频率 (Hz)
amplitude  = 1.0;     % 峰峰值电压 (Vpp)
offset     = 0.0;     % 直流偏置 (V)
...
%% [新增] 将扫频结果导出到 TXT 文件
filename = 'sweep_result.txt';
fileID = fopen(filename, 'w');
if fileID ~= -1
    fprintf(fileID, 'Frequency(Hz),MeasuredVpp(V)\n');
    for idx = 1:length(freqList)
        fprintf(fileID, '%.0f,%.6f\n', freqList(idx), measuredVpp(idx));
    end
    fclose(fileID);
    fprintf('✅ 扫频结果已保存至: %s\n', filename);
else
    fprintf('⚠️ 无法创建 TXT 文件: %s\n', filename);
end
